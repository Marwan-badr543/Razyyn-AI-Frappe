...
class AccountantAgentChat {
	...
	async render_chat_view() {
		...
		// 'auto': the router reads the question and picks the desk itself —
		// the hybrid default. A person who wants a specific desk can still
		// pick one explicitly from the same dropdown; nothing here takes
		// that away, it only stops "which desk do I even want" from being
		// the first question a new chat asks.
		this.agent_selector = new AgentSelector({ default_agent: 'auto' });
