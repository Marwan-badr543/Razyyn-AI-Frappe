...
def assert_session_is_agent_user() -> str:
	...
	if session_user != AGENT_USER:
		raise NotAgentSessionError(_("This endpoint may only be used by the Accountant Agent account."))
	return session_user
...
def _permission_from_the_erp(doctype: str, action: str) -> DocTypePermission:
	...
	try:
		if frappe.get_meta(doctype).issingle:
			raise DocTypeNotAllowedError(
				_("{0} is a settings page rather than a document, so the agent " "does not write it.").format(
					doctype
				),
		...
		raise DocTypeNotAllowedError(
			_("The agent could not read the definition of {0} in this system.").format(doctype),
...
def search_documents(
	...
	if len(wanted) > MAX_SEARCH_DOCTYPES:
		raise MissingParameterError(
			_("At most {0} document types may be searched at once.").format(MAX_SEARCH_DOCTYPES)
		)
...
def _search_one_doctype(
	...
	available = {df.fieldname for df in meta.fields} | {"name", "docstatus", "modified", "creation", "owner"}
	...
	try:
		...
		matched = count_link_candidates(doctype, filters, or_filters) if len(rows) >= limit else len(rows)
	except frappe.PermissionError:
		# A legitimate customer configuration, not an error: this agent user
...
def _row_party(row: dict) -> str:
	for fieldname in ("customer_name", "customer", "supplier_name", "supplier", "party_name", "party"):
		value = row.get(fieldname)
		if value:
			return str(value)
	return ""
...
def _only_the_caller_s_fields(payload: dict) -> dict:
	...
	for key, value in payload.items():
		if str(key).strip().lower() in _CALLER_MUST_NOT_SET:
			...
			continue
		if isinstance(value, list):
			cleaned[key] = [_only_the_caller_s_fields(row) if isinstance(row, dict) else row for row in value]
		else:
			cleaned[key] = value
	return cleaned
...
def update_existing_document(
	...
	return _mutate_existing(
		...
		verify=lambda doc: _values_the_document_did_not_take(
			doc,
			_only_the_caller_s_fields(payload),
		),
	)
...
def _change_a_draft(doctype: str, docname: str, payload: dict):
	...
	if read_docstatus(doctype, docname) != 0:
		raise WriteRejectedError(
			_(
				"{0} {1} is not a draft, so its contents cannot be changed. A "
	...
	try:
		...
		if not_taken:
			...
			frappe.logger("accountant_agent").info(
				...
				doctype,
				doc.name,
				", ".join(e["field"] for e in not_taken),
			)
...
def _run_one_write(
	...
	if action == "update":
		return update_existing_document(doctype=doctype, docname=docname, payload=payload, **common)
	if action == "submit":
	...
	return amend_existing_document(doctype=doctype, docname=docname, payload=payload, **common)
...
def _batch_skip(entry: dict, ordinal: int, missing: Sequence[int]) -> dict:
	...
	return {
		...
		"error_message": _("Not attempted: it was built from document {0}, which was not recorded.").format(
			needed
		),
	}
