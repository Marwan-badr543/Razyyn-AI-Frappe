...
WHAT WENT WRONG WITHOUT THIS
	The schema reply already says which rows of a table are real
	(`posted_filter`: `docstatus = 1`, `is_cancelled = 0`). It said so as
	ADVICE, to a model that then wrote whatever SQL it liked. Forget the line,
	bury it in an OR, put it in the wrong subquery, and cancelled rows enter
	the total. A live audit reported 148,500 EGP of supplier payments
	"recorded twice" whose live total was 0.00 — every row behind the figure
	was cancelled, and the query that found them had no filter on it.
...
WHY THE TABLE IS REPLACED AND THE WHERE CLAUSE IS NOT INSPECTED
	...
	aliased to the same name:
		...
		SELECT account, SUM(debit) FROM `tabGL Entry` WHERE ... GROUP BY ...

	becomes

		SELECT account, SUM(debit)
		FROM (SELECT * FROM `tabGL Entry` WHERE is_cancelled = 0) AS `tabGL Entry`
		WHERE ... GROUP BY ...

	The model's WHERE, JOIN, GROUP BY and subqueries all still work, because the
	alias keeps the original name; nothing written around the table can reach
	a cancelled row, because from the statement's point of view it does not
	exist; and it holds even when the model does not know the column exists.

WHY A PARSER, AND NOT A REGULAR EXPRESSION
	A table appears in a FROM, in a JOIN, inside a CTE, inside a derived table,
	on both sides of a UNION, and with or without an alias. The read-only guard
	gets away with regular expressions because "does this statement contain a
	forbidden word" is a question about the text; "which table references does
	this statement contain" is a question about the tree. `sqlglot` parses the
	statement, every `Table` node is visited, and the statement is written back
	in the same dialect.
...
Pure on purpose: no `frappe` import. What a table's live rows are is asked
of a callback, so this file is the same on every ERP app and is tested with
no site behind it.
"""
...
def exclude_dead_rows(sql: str, dialect: str, filter_for: FilterFor) -> tuple[str, dict[str, str], bool]:
	...
	Args:
		...
		dialect: the `sqlglot` dialect name — "mysql" for a Frappe site,
			"postgres" for an Odoo one.
	...
	"""
	if sqlglot is None:
		logger.warning("sqlglot is not installed; cancelled rows are not excluded from this read.")
		return sql, {}, False
...
def _predicate_for(name: str, filter_for: FilterFor) -> Optional[str]:
	...
	describe (a CTE alias, a junction table, a catalogue view) is a table
	whose every row counts.
	"""
	try:
...
def read_flag(value) -> bool:
	...
	"true", "1" or "yes". Anything else — absent, "false", "0", "" — is False,
	which is the safe default: the exclusion is on unless it was asked to be
	off.
	"""
